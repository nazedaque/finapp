...
def resolve_score(price, buy, exit_, note, score_sheet) -> float | None:
    """Privilégie le score recalculé, puis le score du Sheet en repli."""
    computed = compute_score(compute_ratio(price, buy, exit_), note)
    return computed if computed is not None else finite_float(score_sheet)
